...
vmprint(pagetable_t pagetable) {
  pagetable_t subpage, subsubpage;
  printf("page table %p\n", pagetable);
  // there are 2^9 = 512 PTEs in a page table.
  for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    if (pte & PTE_V) {
      subpage = (pagetable_t)PTE2PA(pte);
      printf("..%d: pte %p pa %p\n", i, pte, subpage);
      for (int j = 0; j < 512; j++) {
        pte = subpage[j];
        if (pte & PTE_V) {
          subsubpage = (pagetable_t)PTE2PA(pte);
          printf(".. ..%d: pte %p pa %p\n", j, pte, subsubpage);
          for (int k = 0; k < 512; k++) {
            pte = subsubpage[k];
            if (pte & PTE_V) {
...
/*
 * add a mapping to the copy of kernel page table.
 */
...
/*
 * create a kernal page table for the process.
 */
 ...
 */
void
ukvmcopy(pagetable_t kpagetable, pagetable_t upagetable, uint64 newsz, uint64 oldsz)
{
  uint64 va;
  pte_t *upte;
  pte_t *kpte;
  ...
  if(newsz >= PLIC)
    panic("ukvmcopy: newsz too large");
